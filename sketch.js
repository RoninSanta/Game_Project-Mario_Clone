...
// Pre- Load Function to load Assets
//===================================
/*
   PreLoad function is used to
   - load the formats of the sound files that willl be used in
   soundFormats ('mp3' , 'wav'); ==> set as mp3 and .wav
   - find the find in the specific assets folder and assign it to a variable
   - that variable is loaded before anything else and will activate whenever its being called
     in a function as .play();
  - .stop(); is to stop a specific file from being play such that it does not plays on top
    of existing sound
  - rate() is used to control the rate in which the Background music is being played such that
    for each collectable is taken, the faster the music will go creating an urgency and
    interesting player experience
*/

// *EXTENSION 2 ADD SOUNDS
// All sound and effects are Loyalty-free from https://freesound.org
//==========================
...
/* - Creating Enemies is the hardest part of this Assignment
 - The Cousera provided example such as a moving a enemy with limited movement
 - Decided to improve on that example and create a roaming enemy that moves in a circular pattern instead of a linear one
 - Since the enemy move in a cicular pattern I decided to improve on the graphics by creating a Guillotine design - inspired by 8bit retro games. Hardest part is to get the
   dist. of the blades and the player close enough such that the point of contact is correct
*/

/* Creates CONSTRUCTOR Trap Function which is the enemies

- The bladeVector is created for direction in which the Blades is swinging 2*Pi = 360 degrees clockwise
- In the draw function is the hardest part of the code as it has to decide the angle as well as the rotation of the blade
  - Create 3 Vectors createVector() to represent the Direction of movement of the Blades

  - using rotate(this.rotateSpeed) we can allow the blades to rotate in a constate speed at 360 degress around the center
    This technique i learned it when watching the youtube on traslating and rotation function tutorial and wanted to impliment it on my project every since and glad it worked out well

  -angle += rotatespeed ; this is important so that all the Blades will move in same speed and will contantly increase to create the swinging effect

 -CheckContact function is distance between the Vector and the Character if checkContact < 50 return true to Draw FuntionEnemies for loop,==> lives decreases
   this part is the hardest and require a lot of trial and error to get the dist. close enough such that the point of contact seems real. It reqiures a lot of patience and
   testing to find the correct dist from blade to player.


- this.Blades and this.chain is for graphical purposes and draws the Guillotine design
*/
